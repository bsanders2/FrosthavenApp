...
class MonsterFrame():
    def __init__(self, name='Default', monster=None, i=0):
        ...
        self.spin = sg.OptionMenu([x for x in range(health+1)], health, key='Spin'+str(i),size=(20,20))
...
def monsterMoveImages(window, values, frames, delete_i):
    for i in range(delete_i, n_rows*n_cols):
        ...
        # frames[i].DefaultValue = frame.spin.DefaultValue
        # frames[i].Values = frame.spin.Values
        frames[i].spin.DefaultValue = val
